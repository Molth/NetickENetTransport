...
namespace Netick.Transport
{
    ...
    public unsafe struct ENetTransportPacket
    {
        public ENetPeer* Peer;
        public ENetPacket* Packet;

        public ENetTransportPacket(ENetPeer* peer, ENetPacket* data)
        {
            Peer = peer;
            Packet = data;
        }

        public static ENetTransportPacket Create(ENetPeer* peer, byte* data, int dataLength, ENetPacketFlag flags)
        {
            var packet = enet_packet_create(data, (nuint)dataLength, (uint)flags);
            return new ENetTransportPacket(peer, packet);
        }
    }
    ...
    public sealed unsafe class ENetConnection : TransportConnection
    {
        ...
        public override int Mtu => (int)ENet.ENET_HOST_DEFAULT_MTU;
    ...
    public sealed unsafe class ENetTransport : NetworkTransport
    {
        ...
        private NativeConcurrentQueue<ENetTransportPacket> _outgoings;
        private BitBuffer _buffer;

        internal void Send(ENetPeer* peer, IntPtr ptr, int length, ENetPacketFlag flags) => _outgoings.Enqueue(ENetTransportPacket.Create(peer, (byte*)ptr, length, flags));
        ...
        public override void Connect(string address, int port, byte[] connectionData, int connectionDataLength)
        {
            ...
            enet_address_set_host_ip(&enetAddress, address);
        ...
        public override void Run(RunMode mode, int port)
        {
            ...
            _outgoings = new NativeConcurrentQueue<ENetTransportPacket>(1, 2);
            enet_initialize();
            ...
            else
            {
                ...
                ENetAddress enetAddress;
                enet_address_set_host_ip(&enetAddress, "::0");
                enetAddress.port = (ushort)port;
                _host = enet_host_create(&enetAddress, (nuint)Engine.MaxClients, 0, 0, 0);
            }
            ...
            while (_incomings.TryDequeue(out var netEvent))
            {
                ...
                switch (netEvent.type)
                {
                    ...
                    case ENetEventType.ENET_EVENT_TYPE_RECEIVE:
                        ...
                        if (connection != null)
                        {
                            _buffer.SetFrom(netEvent.packet->data, (int)netEvent.packet->dataLength,
                                (int)netEvent.packet->dataLength);
